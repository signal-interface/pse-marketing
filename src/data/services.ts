...
export const serviceData: Record<string, ServiceData> = {
  ...
  'payroll-processing': { // boundary-lint-allow: prohibited-execution-nominal -- legacy public slug; rename tracked separately, requires 308 + re-index
    ...
    body: 'PSE\'s governed payroll runs sit on top of CHAP AI — the pre-run compliance engine that validates every employee record against the applicable federal statutory ruleset before the payroll commits. This is not a post-processing audit layer. It is a blocking gate: violations are surfaced, explained in plain language, and resolved before payroll closes. PSE validates and governs — your existing payroll processor executes. The result is payroll that passes its own compliance audit on every run.',
    features: [
      ...
      { title: 'Guided correction workflow', body: 'When CHAP AI flags a violation, the correction interface surfaces the specific employee records, the applicable statute, and the exact change required.' },
  ...
  'tax-compliance': {
    ...
    metaDescription: 'Payroll tax compliance audit and tax notice support for multi-state employers. Regulatory change monitoring, deposit timing validation, and structured response plans for IRS and state agency notices.',
    body: 'PSE audits your payroll tax position across federal, state, and local jurisdictions — identifying exposure before it triggers a penalty. Regulatory changes are tracked and reflected in your compliance rules. When a tax notice arrives from the IRS or a state agency, PSE researches the issue, scopes the liability, and delivers a structured response plan. CHAP AI validates deposit timing and amounts against IRC §6656 requirements on every payroll run — flagging issues before they become notices.',
    features: [
      ...
      { title: 'Regulatory change monitoring', body: 'Rate changes, new requirements, and statutory updates at the federal and state level are tracked and reflected in your compliance rules.' },
  ...
  'workforce-analytics': {
    ...
    ctaSource: 'workforce-analytics-interest',
    related: [
      {
        slug: 'payroll-processing', // boundary-lint-allow: prohibited-execution-nominal -- legacy public slug; rename tracked separately, requires 308 + re-index
        name: 'Governed Payroll Runs',
  ...
  'system-integration': {
    ...
    ctaLabel: 'Notify Me When Available',
    ctaSource: 'system-integration-interest',
    related: [
      {
        slug: 'payroll-processing', // boundary-lint-allow: prohibited-execution-nominal -- legacy public slug; rename tracked separately, requires 308 + re-index
        name: 'Governed Payroll Runs',
